...
For testing, I first verified model accuracy on normal test images, achieving over 98% accuracy. I then used an attack success rate (Backdoored images that fooled the model/total backdoored images) test similar to the course provided code from the course (Appendix C) to verify how effective the trigger was, achieving over 99% accuracy.
...
Additionally, given model ASR essentially stays the same as shown in the results tab across multiple trigger values, assess there to be no need to use an extremely high trigger value (of note, as shown below, even with a high trigger value, the difference across images is difficult to see)
